...
from photogrammetry_importer.operators.colmap_import_op import ImportColmapOperator
from photogrammetry_importer.operators.meshroom_import_op import ImportMeshroomOperator
...
def colmap_import_operator_function(self, context):
    self.layout.operator(ImportColmapOperator.bl_idname, text="Colmap Import (model/workspace)")

def meshroom_import_operator_function(self, context):
    self.layout.operator(ImportMeshroomOperator.bl_idname, text="Meshroom Import (.sfm/.json/.mg)")

def mve_import_operator_function(self, context):
    self.layout.operator(ImportMVEOperator.bl_idname, text="MVE Import (workspace)")

def open3d_import_operator_function(self, context):
    self.layout.operator(ImportOpen3DOperator.bl_idname, text="Open3D Import (.log/.json)")

def opensfm_import_operator_function(self, context):
    self.layout.operator(ImportOpenSfMOperator.bl_idname, text="OpenSfM Import (.json)")
...
def colmap_export_operator_function(self, context):
    self.layout.operator(ExportColmapOperator.bl_idname, text="Colmap Export (folder)")

def visualsfm_export_operator_function(self, context):
    self.layout.operator(ExportNVMOperator.bl_idname, text="VisualSfM Export (.nvm)")
...
def register_importers(import_prefs):
    register_importer(import_prefs.colmap_importer_bool, ImportColmapOperator, colmap_import_operator_function)
    register_importer(import_prefs.meshroom_importer_bool, ImportMeshroomOperator, meshroom_import_operator_function)
    register_importer(import_prefs.mve_importer_bool, ImportMVEOperator, mve_import_operator_function)
...
def unregister_importers():
    ...
    unregister_importer(ImportMeshroomOperator, meshroom_import_operator_function)
...
def register_exporters(export_prefs):
    register_exporter(export_prefs.colmap_exporter_bool, ExportColmapOperator, colmap_export_operator_function)
    register_exporter(export_prefs.visualsfm_exporter_bool, ExportNVMOperator, visualsfm_export_operator_function)
