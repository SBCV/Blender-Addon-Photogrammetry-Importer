...
class OpenSfMJSONFileHandler:
    ...
    @staticmethod
    def convert_intrinsics(json_camera_intrinsics, relative_fp, suppress_distortion_warnings, op):
        ...
        radial_distortion = [json_camera_intrinsics['k1'], json_camera_intrinsics['k2']]
        ...
        if projection_type == 'perspective':
            ...
            cx = width/2
            cy = height/2
            log_report('WARNING','Principal point not provided, setting it to the image center.', op)
            ...
            if fx != fy:
                log_report('WARNING','Focal length in x and y direction differs, setting it to the average value.', op)
    ...
    def parse_cameras(json_data, image_dp, image_fp_type, suppress_distortion_warnings, op):
        ...
        for view_name in views:
            ...
            focal_length, cx, cy, width, height, radial_distortion = OpenSfMJSONFileHandler.convert_intrinsics(
                json_cameras_intrinsics[intrinsic_key], camera._relative_fp, suppress_distortion_warnings, op)
    ...
    @staticmethod
    def parse_opensfm_file(input_opensfm_fp, image_dp, image_fp_type, suppress_distortion_warnings, reconstruction_idx, op):
        ...
        cams = OpenSfMJSONFileHandler.parse_cameras(
            reconstruction_data, image_dp, image_fp_type, suppress_distortion_warnings, op)
        points = OpenSfMJSONFileHandler.parse_points(
            reconstruction_data, op)
