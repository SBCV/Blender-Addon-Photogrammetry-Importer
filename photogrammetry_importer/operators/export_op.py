...
class ExportOperator(bpy.types.Operator):
    ...
    def get_calibration_mat(self, blender_camera):
        ...
        log_report('INFO', 'render_resolution_width: ' + str(render_resolution_width), self)
        log_report('INFO', 'render_resolution_height: ' + str(render_resolution_height), self)
        log_report('INFO', 'focal_length_in_pixel: ' + str(focal_length_in_pixel), self)
    ...
    def get_computer_vision_camera_matrix(self, blender_camera):
        ...
        if not np.allclose(tuple(blender_camera.scale), (1, 1, 1)):
            log_report('ERROR', 'blender_camera.scale: ' + str(blender_camera.scale), self)
            assert False
    ...
    def export_selected_cameras_and_vertices_of_meshes(self, odp):
        log_report('INFO', 'export_selected_cameras_and_vertices_of_meshes: ...', self)
        cameras = []
        points = []
        ...
        for obj in bpy.context.selected_objects:
            if obj.type == 'CAMERA':
                ...
                camera_matrix_computer_vision = self.get_computer_vision_camera_matrix(obj)
                ...
                if obj.data is not None:
                    ...
                    for vert in obj.data.vertices:
                        ...
                        point_index += 1
                    points += obj_points
        log_report('INFO', 'export_selected_cameras_and_vertices_of_meshes: Done', self)
